...
string TestsInfo::printInfo()const {
	stringstream ss;
	ss << "\n\n Student number:" << this->num << endl << "Number of completed works:" << this->works << endl
		<< "Average volume of work:" << this->pages << endl << "GPA:" << this->mark << endl << "Surname :"
		<< this->surname;
...
void TestsInfo::setGraph() {
	string chart;
	cout << "Enter chart type :";
	cin >> chart;
	graph.setType(chart);
}

void TestsInfo::printGraph() {
	cout << graph.getType();
}