...
Textbook::Textbook() :num(0), pages(rand() % 100 + 180), grade(rand() % 11 + 1), cost(rand() % 120 + 200), year_of_release(rand() % 15 + 2000){
	author = (char *) operator new (sizeof(char) * 64);
	cout << "The constructor by default worked here" << endl;
}

Textbook::Textbook(int num, int pages, int grade, int cost, int year_of_release, char* author) :
	...
	strcpy_s(this->author, 64, author);
...
void Textbook::setAuthor(char* author) {
	strcpy_s(Textbook::author, 64, author);
}
char *Textbook::getAuthor() {
