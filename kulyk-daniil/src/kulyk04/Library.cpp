...
Library::Library(int size) : size(size), new_textbook(), backlist(0) {
	...
	regex sur("(([A-Z])([a-z]+)" "([A-Z])([a-z]+))");
	char buff[30];
	ifstream fin("C:\\kulyk-daniil\\src\\kulyk02\\kulyk02\\AuthorsNames.txt");
	stringstream s1;

	for (int i = 0; i < size; i++) {
		fin.getline(buff, 30);
		/*if (regex_match(buff, sur)) */
			backlist[i].setInfo(i + 1, rand() % 100 + 180, rand() % 11 + 1, rand() % 100 + 220, rand() % 15 + 2000, buff);

	}
	fin.getline(buff, 30);
	if (regex_match(buff, sur))
	new_textbook.setInfo(size + 1, rand() % 100 + 180, rand() % 11 + 1, rand() % 100 + 220, rand() % 15 + 2000, buff);
...
void Library::printWithRegex()const {

	regex str("(([A-Z])([a-z]+) ([A-Z])([a-z]+))");

	for (int i = 0; i < size; i++) {
		if (regex_match(backlist[i].getAuthor(), str)) {
			cout << backlist[i].getAuthor() << endl;
		}
	}
}