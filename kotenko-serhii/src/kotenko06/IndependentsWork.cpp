...
void IndependentsWork::read_from_file(std::string *person) {
...
void IndependentsWork::new_array(std::string *person) {
...
void IndependentsWork::add_elem(int amount, int written, int mark, std::string person) {
	...
	mas[size - 1].set_data(amount, written, mark, person);
...
void IndependentsWork::write_to_file() {
	...
	for (int i = 0; i < size; i++) {
		...
		//fout << "Student homework : " << arr[i].getHomework() << std::endl << std::endl;
	}
	fout.close();
}
void IndependentsWork::delete_array() {
	delete[]arr;
}
