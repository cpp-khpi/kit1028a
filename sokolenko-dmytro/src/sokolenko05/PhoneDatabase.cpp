...
size_t PhoneDatabase::getSize() const { return size; }
...
PhoneDatabase::PhoneDatabase(size_t newSize, Phone* newPhoneArray) : 
	size(newSize)
{
	delete[] phoneArray;
...
void PhoneDatabase::setPhoneDatabaseInfo(size_t newSize, Phone* newPhoneArray)
{
	size = newSize;
...
bool PhoneDatabase::comparisonPhoneArray(const Phone* otherPhoneArr, size_t otherSize) const
{
	if (size != otherSize) {
		return false;
	}
